...
function ProductEdit() {
  ...
  const addVariant = () => setVariants([...variants, {
    id: "new_" + Date.now(), sku: form.sku + "-V" + (variants.length + 1),
    attrs: { color: "", size: "" }, price_cny: form.price_cny ?? 0, stock: 0, is_active: true,
  }]);
  const updateVariant = (idx: number, patch: any) => {
    const n = [...variants]; n[idx] = { ...n[idx], ...patch }; setVariants(n);
  };
  ...
  const updateImage = (i: number, v: string) => {
    const n = [...form.images]; n[i] = v; setForm({ ...form, images: n });
  };
  const removeImage = (i: number) => setForm({ ...form, images: form.images.filter((_: any, idx: number) => idx !== i) });
  ...
  const updateBlock = (i: number, patch: any) => {
    const n = [...form.detail_blocks]; n[i] = { ...n[i], ...patch }; setForm({ ...form, detail_blocks: n });
  };
  const removeBlock = (i: number) =>
    setForm({ ...form, detail_blocks: form.detail_blocks.filter((_: any, idx: number) => idx !== i) });

  if (!isNew && q.isLoading) return <div className="grid h-[60vh] place-items-center"><Loader2 className="h-6 w-6 animate-spin text-slate-500"/></div>;

  return (
    <div className="mx-auto max-w-5xl p-6">
      <button onClick={() => navigate({ to: "/admin/shop/products" })}
        className="mb-3 inline-flex items-center gap-1 text-xs text-slate-400 hover:text-white">
        <ArrowLeft className="h-3 w-3"/>返回商品列表
      </button>
      <div className="mb-5 flex items-center justify-between">
        <h1 className="font-display text-2xl font-bold inline-flex items-center gap-2">
          <PackageOpen className="h-5 w-5 text-blue-400"/>{isNew ? "新增商品" : "编辑商品"}
        </h1>
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          ...
          <Field label="状态">
            <select value={form.status} onChange={(e) => setForm({ ...form, status: e.target.value })}
              className="w-full rounded-md border border-white/10 bg-white/5 px-2 py-1.5 text-sm [&>option]:bg-[#0E1626]">
              <option value="draft">草稿</option><option value="active">在售</option><option value="archived">下架</option>
            </select>
          </Field>
          <Field label="价格 CNY"><Input type="number" value={String(form.price_cny ?? "")} onChange={v => setForm({ ...form, price_cny: Number(v) || 0 })}/></Field>
      ...
      <Section title="媒体 / 图片库（上传到 shop-media 桶）">
        ...
        <div className="mt-3">
          <div className="mb-2 flex items-center justify-between">
            ...
            <button onClick={addImage} className="inline-flex items-center gap-1 rounded border border-white/10 bg-white/5 px-2 py-1 text-xs hover:bg-white/10"><Plus className="h-3 w-3"/>添加图片</button>
          </div>
            ...
            {form.images.map((url: string, i: number) => (
              <div key={i} className="rounded border border-white/5 bg-white/[0.03] p-2">
                <div className="mb-1 flex items-center justify-between text-[10px] uppercase tracking-wider text-slate-400">
                  <span>#{i + 1}</span>
                  <button onClick={() => removeImage(i)} className="text-rose-400 hover:text-rose-300"><Trash2 className="h-3 w-3"/></button>
                </div>
      ...
      <Section title="详情页内容（图文 / 视频）">
        <div className="mb-2 flex gap-2">
          <button onClick={() => addBlock("image")} className="inline-flex items-center gap-1 rounded border border-white/10 bg-white/5 px-2 py-1 text-xs hover:bg-white/10"><ImageIcon className="h-3 w-3"/>图片块</button>
          <button onClick={() => addBlock("video")} className="inline-flex items-center gap-1 rounded border border-white/10 bg-white/5 px-2 py-1 text-xs hover:bg-white/10"><Video className="h-3 w-3"/>视频块</button>
          <button onClick={() => addBlock("text")} className="inline-flex items-center gap-1 rounded border border-white/10 bg-white/5 px-2 py-1 text-xs hover:bg-white/10"><FileText className="h-3 w-3"/>文本块</button>
        </div>
          ...
          {form.detail_blocks.map((b: any, i: number) => (
            <div key={i} className="rounded border border-white/5 bg-white/[0.03] p-2">
              <div className="mb-1.5 flex items-center justify-between text-[10px] uppercase tracking-wider text-slate-400">
                ...
                <button onClick={() => removeBlock(i)} className="text-rose-400 hover:text-rose-300"><Trash2 className="h-3 w-3"/></button>
              </div>
              {b.type === "text" ? (
                <textarea value={b.content ?? ""} onChange={e => updateBlock(i, { content: e.target.value })} rows={3}
                  className="w-full rounded-md border border-white/10 bg-white/5 px-2 py-1.5 text-sm focus:border-brand focus:outline-none"/>
              ) : (
                <MediaUpload value={b.url ?? ""} onChange={(v) => updateBlock(i, { url: v })} accept={b.type === "video" ? "video/*" : "image/*"}/>
              )}
            </div>
          ))}
        </div>
      ...
      <Section title="商务设置（HS Code / 厂家 / 采购模式 / 关税 / 末端派送费）">
        <div className="grid gap-3 sm:grid-cols-2">
          <Field label="HS Code"><Input value={form.hs_code ?? ""} onChange={v => setForm({ ...form, hs_code: v })}/></Field>
          <Field label="生产厂家（仅后台可见）"><Input value={form.manufacturer ?? ""} onChange={v => setForm({ ...form, manufacturer: v })}/></Field>
          <Field label="货物类型">
            <select value={form.cargo_type ?? "general"}
          ...
          <Field label="允许的采购模式" full>
            <div className="flex flex-wrap gap-3 text-sm">
              <label className={`inline-flex items-center gap-2 rounded-md border px-3 py-1.5 cursor-pointer ${form.allow_personal ? "border-brand bg-brand/10" : "border-white/10 bg-white/5"}`}>
                <input type="checkbox" checked={!!form.allow_personal}
                  onChange={(e) => setForm({ ...form, allow_personal: e.target.checked })}/>
                个人采购
              </label>
              <label className={`inline-flex items-center gap-2 rounded-md border px-3 py-1.5 cursor-pointer ${form.allow_business ? "border-brand bg-brand/10" : "border-white/10 bg-white/5"}`}>
                <input type="checkbox" checked={!!form.allow_business}
      ...
      {form.allow_personal && (
        <Section title="个人采购运费公式">
          <div className="grid gap-3 sm:grid-cols-3">
            <Field label="计费方式">
              <select value={form.personal_freight_mode}
            ...
            {form.personal_freight_mode === "per_unit" ? (
              <>
                ...
                <Field label="单件预设运费 · 海运 CNY">
                  <Input type="number" value={String(form.personal_per_unit_freight_sea_cny ?? 0)}
      ...
      <Section title="线路匹配（按 采购模式 × 运输方式 各选一条线路）">
        {routes.length === 0 ? (
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            {([
            ...
            ] as const).map(([mode, method, field, label]) => {
              ...
              return (
                <Field key={field} label={label + (enabled ? "" : "（未启用该采购模式）")}>
                  <select value={form[field] ?? ""} disabled={!enabled}
                    ...
                    {candidates.map((r) => (
                      <option key={r.code} value={r.code}>{r.code} · {r.name_zh}{r.destination_code ? ` → ${r.destination_code}` : ""}</option>
                    ))}
                  </select>
                </Field>
              );
      ...
      <Section title="规格变体 / SKU">
        <div className="mb-3 flex items-center justify-end">
          <button onClick={addVariant} className="inline-flex items-center gap-1 rounded-md border border-white/10 bg-white/5 px-2 py-1 text-xs hover:bg-white/10">
            <Plus className="h-3 w-3"/>新增变体
          </button>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-xs">
            <thead className="text-left text-[10px] uppercase tracking-wider text-slate-500">
              <tr>
                <th className="py-2 pr-2">SKU</th><th className="py-2 pr-2">颜色</th><th className="py-2 pr-2">尺寸</th>
                <th className="py-2 pr-2">价格</th><th className="py-2 pr-2">库存</th><th></th>
              </tr>
            </thead>
            <tbody className="divide-y divide-white/5">
              {variants.length === 0 && <tr><td colSpan={6} className="py-4 text-center text-slate-500">尚无变体</td></tr>}
              {variants.map((v, i) => (
                <tr key={v.id ?? i}>
                  <td className="py-1.5 pr-2"><MiniInput value={v.sku} onChange={x => updateVariant(i, { sku: x })}/></td>
                  <td className="py-1.5 pr-2"><MiniInput value={v.attrs?.color ?? ""} onChange={x => updateVariant(i, { attrs: { ...v.attrs, color: x } })}/></td>
                  <td className="py-1.5 pr-2"><MiniInput value={v.attrs?.size ?? ""} onChange={x => updateVariant(i, { attrs: { ...v.attrs, size: x } })}/></td>
                  <td className="py-1.5 pr-2"><MiniInput type="number" value={String(v.price_cny ?? "")} onChange={x => updateVariant(i, { price_cny: Number(x) || 0 })}/></td>
                  <td className="py-1.5 pr-2 text-right text-slate-400">{v.stock}</td>
                  <td className="py-1.5 text-right">
                    <button onClick={() => removeVariant(i)} className="text-rose-400 hover:text-rose-300"><Trash2 className="h-3.5 w-3.5"/></button>
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        {!isNew && variants.length > 0 && (
          <p className="mt-3 text-[11px] text-slate-500">⚠️ 库存请通过「库存流水」页面入库/调整，编辑变体的 stock 字段不会触发库存流水。</p>
        )}
      </Section>
    </div>
  );
...
function Section({ title, children }: any) {
  return <div className="mt-4 rounded-2xl border border-white/5 bg-white/[0.02] p-5">
