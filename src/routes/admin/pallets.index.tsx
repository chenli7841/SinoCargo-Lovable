      ...
      <div className="overflow-x-auto rounded-2xl border border-white/5 bg-white/[0.02]">
        <table className="w-full text-sm">
          <thead className="bg-white/[0.03] text-left text-[11px] uppercase text-slate-400">
            <tr><th className="px-4 py-2.5">托盘号</th><th>线路</th><th>客户</th><th>目的地</th><th>状态</th><th>付款</th><th>计费重</th><th>总费用 (CAD)</th><th>所属批次</th><th>创建</th><th></th></tr>
          </thead>
          <tbody className="divide-y divide-white/5">
            ...
            {q.data?.items.map((p: any) => {
              ...
              <tr key={p.id} className="hover:bg-white/[0.03]">
                <td className="px-4 py-3 font-mono text-xs text-slate-200">{p.pallet_no}</td>
                <td className="text-xs text-slate-400">{p.route_code ?? "—"}</td>
                <td className="text-xs font-mono">{p.customer_code ?? "—"}</td>
                <td className="text-xs text-slate-400">{p.destination_code ?? "—"}</td>
                <td className="text-xs">{p.status}</td>
                <td><PaymentBadge s={p.payment_status}/></td>
                <td className="text-xs" title={`实际重量 ${actualW} kg（自身 ${selfW} + 下属 ${childW}）· 自身计费重 ${selfChg} · 下属计费重之和 ${childChg} · 体积重 ${volW || 0} kg · 采用 ${p.customer_code ? "自身" : "下属之和"}`}>
                  {chargeW ? `${chargeW} kg` : (actualW ? `${actualW} kg` : "—")}
                </td>
                <td className="text-xs font-mono text-emerald-300" title={p.has_customer
                  ? `客户号模式 = 自身运费 CA$${(p.self_freight_cad ?? 0).toFixed(2)} + 关税 CA$${(p.child_customs_cad ?? 0).toFixed(2)} + 保险 CA$${(p.child_insurance_cad ?? 0).toFixed(2)} + 附加费 CA$${(p.surcharge_cad ?? 0).toFixed(2)}`
                  : `无客户号模式 = 下属运费 CA$${(p.child_freight_cad ?? 0).toFixed(2)} + 关税 CA$${(p.child_customs_cad ?? 0).toFixed(2)} + 保险 CA$${(p.child_insurance_cad ?? 0).toFixed(2)} + 清关费 CA$${(p.clearance_fee_cad ?? 0).toFixed(2)} + 附加费 CA$${(p.surcharge_cad ?? 0).toFixed(2)}`}>
                  CA${(p.total_fee_cad ?? 0).toFixed?.(2) ?? "0.00"}
                </td>
                <td className="text-xs font-mono">{p.batch_no ? <Link to="/admin/batches/$batchId" params={{ batchId: p.batch_id }} className="text-brand hover:underline">{p.batch_no}</Link> : "—"}</td>
