...
function InvoicesPage() {
  ...
  const listQ = useQuery({ queryKey: ["admin-invoices", page, status, q, userId], queryFn: () => fetchList({ data: { page, pageSize: 20, status: status || undefined, q: q || undefined, userId: userId || undefined } }) });
    ...
    <div className="mx-auto max-w-7xl p-6">
      <div className="mb-5 flex items-center justify-between gap-3">
        <div>
          <h1 className="font-display text-2xl font-bold inline-flex items-center gap-2">
            <FileText className="h-5 w-5 text-blue-400" />账单管理
          </h1>
          <p className="mt-1 text-sm text-slate-400">共 {total} 张账单</p>
        </div>
        {isOwner && (
          <div className="flex flex-col items-end gap-1">
            <button onClick={onSeed} disabled={seedBusy}
              ...
              {seedBusy ? <Loader2 className="h-4 w-4 animate-spin" /> : <Database className="h-4 w-4" />}重置并生成测试运单
            </button>
      ...
      {userId && (
        <div className="mb-3 flex items-center justify-between rounded-md border border-blue-500/30 bg-blue-500/10 px-3 py-2 text-xs text-blue-200">
          <span>已按用户筛选账单 · <span className="font-mono">{userId.slice(0, 8)}</span></span>
          <button onClick={() => navigate({ search: {} })} className="rounded border border-white/10 px-2 py-0.5 text-[11px] hover:bg-white/5">清除筛选</button>
        </div>
      )}
      ...
      <div className="mb-3 flex flex-wrap items-center gap-2 text-sm">
        <select value={status} onChange={(e) => { setStatus(e.target.value); setPage(1); }}
          ...
          <option value="unpaid">待付</option><option value="paid">已付</option>
          <option value="overdue">逾期</option><option value="void">作废</option>
        </select>
        ...
        {selected.size >= 2 && (
          <button onClick={async () => {
            ...
            <GitMerge className="h-3.5 w-3.5"/>合并 {selected.size} 张
          </button>
        )}
      </div>
      ...
      <div className="overflow-hidden rounded-2xl border border-white/5 bg-white/[0.02]">
        <table className="w-full text-sm">
          <thead className="bg-white/[0.03] text-left text-[11px] uppercase tracking-wider text-slate-400">
            <tr>
              <th className="px-3 py-2.5"><input type="checkbox"
          ...
          <tbody className="divide-y divide-white/5">
            {listQ.isLoading && <tr><td colSpan={9} className="px-4 py-12 text-center"><Loader2 className="mx-auto h-5 w-5 animate-spin text-slate-500" /></td></tr>}
            {items.length === 0 && !listQ.isLoading && <tr><td colSpan={9} className="px-4 py-12 text-center text-slate-500">暂无账单</td></tr>}
            {items.map((r: any) => (
              <tr key={r.id} className="hover:bg-white/[0.03]">
                <td className="px-3 py-3">
                  <input type="checkbox" disabled={r.status !== "unpaid"} checked={selected.has(r.id)} onChange={() => {
                    const s = new Set(selected);
                ...
                <td className="px-4 py-3 font-mono text-xs">
                  <Link to="/admin/invoices/$invoiceId" params={{ invoiceId: r.id }} className="text-blue-300 hover:underline">{r.invoice_no}</Link>
                </td>
                ...
                <td className="px-4 py-3 text-xs">{r.type}{r.batch_no ? <div className="font-mono text-[10px] text-slate-500">{r.batch_no}</div> : null}</td>
                <td className="px-4 py-3 text-sm font-semibold">¥{Number(r.total_cny).toFixed(2)}</td>
                <td className="px-4 py-3"><span className={`rounded-full border px-2 py-0.5 text-[10px] font-semibold ${STATUS_COLORS[r.status]}`}>{STATUS_LABEL[r.status]}</span></td>
                ...
                <td className="px-4 py-3 text-right">
                  <div className="inline-flex gap-1">
                    {r.status === "unpaid" && (
                      <button onClick={() => onMarkPaid(r.id)} title="标记已付" className="rounded-md border border-emerald-500/30 p-1.5 text-emerald-300 hover:bg-emerald-500/10"><CheckCircle2 className="h-3.5 w-3.5" /></button>
                    )}
                    {r.status !== "void" && r.status !== "paid" && (
                      <button onClick={() => onVoid(r.id)} title="作废" className="rounded-md border border-slate-500/30 p-1.5 text-slate-300 hover:bg-slate-500/10"><Ban className="h-3.5 w-3.5" /></button>
                    )}
                    <button onClick={() => onDelete(r.id)} title="删除" className="rounded-md border border-rose-500/30 p-1.5 text-rose-300 hover:bg-rose-500/10"><Trash2 className="h-3.5 w-3.5" /></button>
                  </div>
                </td>
              </tr>
            ))}
      ...
      <div className="mt-3 flex items-center justify-between text-sm">
        <div className="text-slate-400">第 {page} / {totalPages} 页</div>
        <div className="flex gap-2">
          <button disabled={page <= 1} onClick={() => setPage((p) => p - 1)} className="rounded-md border border-white/10 px-3 py-1.5 disabled:opacity-30 hover:bg-white/5">上一页</button>
          <button disabled={page >= totalPages} onClick={() => setPage((p) => p + 1)} className="rounded-md border border-white/10 px-3 py-1.5 disabled:opacity-30 hover:bg-white/5">下一页</button>
        </div>
      </div>
    </div>
  );
